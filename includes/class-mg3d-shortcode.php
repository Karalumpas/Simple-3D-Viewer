    ...
    public function render_shortcode($atts) {
        try {
            ...
            if (!$post_id || get_post_type($post_id) !== 'mg3d_model') {
                throw new Exception(__('Invalid 3D model ID.', 'mg-3d-productviewer'));
            }

            $model_url = get_post_meta($post_id, '_mg3d_model_url', true);
            if (!$model_url) {
                throw new Exception(__('No 3D model found.', 'mg-3d-productviewer'));
            }

            // Validate URLs
            if (!filter_var($model_url, FILTER_VALIDATE_URL)) {
                throw new Exception(__('Invalid model URL.', 'mg-3d-productviewer'));
            }

            // Validate numeric values
            if (!empty($atts['rotation_speed']) && (!is_numeric($atts['rotation_speed']) || $atts['rotation_speed'] < 1)) {
                $atts['rotation_speed'] = 30;
            }
            if (!empty($atts['zoom_level']) && (!is_numeric($atts['zoom_level']) || $atts['zoom_level'] < 0.5 || $atts['zoom_level'] > 3)) {
                $atts['zoom_level'] = 1.5;
            }
            if (!empty($atts['shadow_intensity']) && (!is_numeric($atts['shadow_intensity']) || $atts['shadow_intensity'] < 0 || $atts['shadow_intensity'] > 1)) {
                $atts['shadow_intensity'] = 1;
            }
            if (!empty($atts['exposure']) && (!is_numeric($atts['exposure']) || $atts['exposure'] < 0 || $atts['exposure'] > 2)) {
                $atts['exposure'] = 1;
            }
            ...
            return '<div class="mg3d-error">' . esc_html($e->getMessage()) . '</div>';
        }
    }
